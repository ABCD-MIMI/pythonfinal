...
class BankAccount:
    def __init__(self, id, name, email, address, account_type,password):
        self.id = id
        self.name = name
        self.email = email
    ...
    def loan(self, amount):
        if self.loans_taken < 2:
            if self.balance >= amount:
                ...
                return "Insufficient funds in your account for this loan."
        else:
            return "You have reached your loan limit."
    ...
    def transfer(self, amount, from_id, to_id):
        ...
        if from_account is not None and to_account is not None:
            if from_account.balance >= amount:
                ...
                return f"{amount} transferred from account ID {from_id} to account ID {to_id}"
            else:
...
class User:
    def __init__(self, name, email, password, id, account_type):
        ...
        self.email = email
        self.password = password
        self.account = BankAccount(id, name, email,password, 'address', account_type)
